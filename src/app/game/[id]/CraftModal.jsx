              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    padding: '10px 12px',
                    borderRadius: 10,
                    border: `1px solid ${active ? rarity.color : T.border}`,
                    background: active ? `${rarity.color}12` : T.bg2,
                    color: active ? rarity.color : T.text,
                    cursor: 'pointer',
                  }}
                >
                  <div style={{ fontWeight: 700 }}>{tier.name}</div>
                  <div style={{ fontSize: 11, color: active ? rarity.color : T.dim, marginTop: 4 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {selectedTier ? (
              <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {(selectedRecipe?.ingredients || []).map(ingredient => (
                      ...
                      >
                        ...
                        <span style={{ color: T.dim }}>x{ingredient.quantity}</span>
                      </div>
                    ))}
                ...
                <div style={{ padding: '14px 16px', borderRadius: 12, border: `1px solid ${T.border}`, background: T.bg2 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: T.dim, marginBottom: 10 }}>合成校验</div>
                  {preview ? (
                    preview.canCraft ? (
                      <div style={{ color: T.green, fontSize: 13 }}>材料齐全，可以开始合成。</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {preview.missing.map((item, index) => (
                          <div key={`${item}-${index}`} style={{ color: T.red, fontSize: 12 }}>
                            • {item}
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ color: T.dim, fontSize: 12 }}>正在计算条件...</div>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                  <button
                    onClick={onClose}
                    style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${T.border}`, background: 'transparent', color: T.dim, cursor: 'pointer' }}
                  >
                    关闭
                  </button>
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: 'none',
                      background: preview?.canCraft ? T.yellow : T.dim3,
              ...
              <div style={{ textAlign: 'center', color: T.dim, padding: 24 }}>请选择一条装备配方</div>
            )}
          </div>
        </div>
      )}
