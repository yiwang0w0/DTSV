...
import { BTN, INPUT, LABEL, WEATHER_OPTIONS } from '../_shared/ui'

const GAME_TYPES = { 0: '个人战', 2: 'PVE', 11: '2v2', 12: '3v3', 13: '4v4', 14: '自由团战' }
...
export default function MapsTab({ maps, setMaps, toast }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: selectedMap ? '320px 1fr' : '1fr', gap: 16 }}>
      {/* 左：地图列表 */}
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {filtered.map(map => (
            <div key={map.map_id} onClick={() => setSelectedMap(map.map_id === selectedMap ? null : map.map_id)}
              style={{ padding: '10px 14px', borderRadius: 10, cursor: 'pointer', transition: 'border-color .15s',
                background: selectedMap === map.map_id ? 'rgba(88,166,255,0.08)' : '#1c2129',
                border: `1px solid ${map.blocked ? '#f85149' : selectedMap === map.map_id ? '#58a6ff' : '#30363d'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 8, fontSize: 11, color: '#8b949e' }}>
                  <span>{map.weather || 'clear'}</span>
                  <span>{GAME_TYPES[map.game_type] || '个人战'}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {sel && (
        <div style={{ background: '#1c2129', borderRadius: 12, border: '1px solid #30363d', padding: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              <label style={LABEL}>危险度 (1-5)</label>
              <input type="number" min={1} max={5} style={INPUT} value={sel.danger_level || 1} onChange={e => update(sel.map_id, { danger_level: Number(e.target.value) })} />
            </div>
