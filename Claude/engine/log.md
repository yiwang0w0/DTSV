...
## ⏸ 停工存档（2026-07-23 · 🧭 停工令）—— 只在对话上下文里、不在别处的判断，全部落在这里

> 🧭 特别要求：「有没有只在你上下文里、没进文档的判断？这比任何代码都难重建。」以下逐条落地。
> **停工点**：工作区干净、全部已推（`main` 见下）。**恢复第一步 = 等 🧭 读 `docs/plan/kaleido/14-field-layout-draft.md` 批四件 → 开 P1 投影重构。`d` 与草案都已交付，不要重做。**

### A. 已交付但交接文件曾误记为「未开工」的两件（我已订正 `Claude/HANDOFF.md`）
- **`d` 负伤流血：已落地**（`0534d9f3`·E2E 91/91×3）。停工令与我的交付消息交错。⚙️ 恢复后 **UPSERT 两行 `game_rules` 即开**，不需要我发版。
- **字段落点草案：已出稿**（doc 14·`e66a9036`）。

### B. 可复用的判据（不是本次结论，是以后还会用到的思考工具）
1. **「未 await 的 promise 在 Vercel serverless 上会随函数冻结被丢掉」** —— 这不是 H3 一次性的诊断，是本仓的**通用判据**。当年 `emitPlayerEvents` 要 `await` 是同一条理由。**凡是"先返回响应、后台再写"的路径，在 serverless 上都要按"这次写可能根本不发生"来设计**。H3 里它更毒的一层是：那条路径同时是**死亡路径**，而死亡后玩家被各 handler `throw` 挡在门外 ⇒ **没有下一个动作来自愈**。⟹ 判据可浓缩为：**fail-late 的前提是"还有下一拍"；终局拍上没有下一拍，fail-late 等于 fail-forever。**
2. **「界面/注释陈述与生效值脱钩」是一类病，不是三个孤立 bug**。本轮同一形状出现四次：`calcItemEffect` 注释写排除式实现写白名单 / admin 显示死列 `atk` 而生效的是 `atk_delta` / `REVEAL_ORDER` 白名单漏键静默吞 / SQL 注释说「旧列是设计记录」而它其实是重复记账。**共同特征：无报错、无痕迹、只有人眼能发现** ⇒ 只能靠 gate 断言防，不能靠评审防。
3. **「单发假设 + 掷骰」= 本仓 flaky gate 的固定模式**（§③ §④ 两次）。凡断言依赖随机结果 → 有界循环 + 把兜底概率算给自己看。
4. **负对照是断言的验收条件，不是可选步骤**。本轮救了三次；其中一次（H3 第一版）证明我自己写的断言**根本没在测它声称测的东西**。

### C. 实测结论（会被"想当然"重新猜错的那种）
- **`initBrRoomLayer` 对 kaleido 永不可达**：`joinRoom` 对 kaleido **先抛**（`'单人对局，无法加入'`），而 BR 判据在那之后 ⇒ kaleido 的 `br.enabled` 恒 `false`、从未初始化。⟹ 教义那句「10x10 已存在只是没观测窗口」不成立：**多人局有完整观测窗口，kaleido 里这套系统没启动**。这决定 `operate` 是「接一个在跑的系统」还是「新建一个」——**工作量差一个量级**。
- **11 个场景持久层候选逐个为什么不合格**：完整表在 `docs/plan/kaleido/13-position-system-recon.md` §3（含每个候选的生命周期与回收时机）。**结论：无一合格**，故必须新建表。
- **「关上那扇门」七段走查：五段在关键路径上且全未落地** ⇒ step1 里程碑从「首次到达某地」改成「关上门」之后**重了一个量级**（原来只需位置判定）。见 doc 13 §6。
- **账号级持久化在历次 E2E 里从未真正执行过**：E2E 用纯内存 uid，而 `profiles.id` 有 FK→`auth.users` ⇒ 那张行根本不存在 ⇒ `.update()` 匹配 0 行**也算成功**。当年「Commit B 已 E2E 验证」只覆盖了房内镜像那一半。
- **`loadGameRules` 是进程级全局 memo、无 TTL、kaleido/多人/admin 共享同一对象身份** ⇒ ①逐关覆盖绝不能写回；②run 中途改 `game_rules` 读不到（这正是我把 `bleed`/`cycleGuarantee` 播种进 `gamevars` 而非每动作现查的原因）。

### D. 我自己判的、待 🧭 追认的两处（不追认也能跑，但语义归属该由 🧭 定）
...
### E. 未解决、且没有 owner 的
- **N7 叙事兜底通道在 `src/` 零实现** ⇒ 法则一的公平性目前**没有任何实现在支撑**。设计稿 doc 12 已出，未落地。
- **重置作业形态未定**（我倾向惰性到期 + 定期清理，见 doc 13 / DDL 末注）。
- **admin 那批改动没有视觉验证**（需管理员登录，输入密码不在我可执行范围）。验收点已进 Kanata 清单。
