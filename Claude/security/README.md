...
**队列**:~~①🔧 ui_unlocks 持久化 DDL 跟审~~ **审毕(不通过 as-is·案②)** —— profiles 有 owner-UPDATE policy(with_check=NULL)→owner 可自改 ui_unlocks 伪造解锁;补防草案 [`scripts/kaleido-ui-unlocks-guard.sql`](../../scripts/kaleido-ui-unlocks-guard.sql)(BEFORE INSERT/UPDATE 触发器·未执行·交 🧭 审)。②🔧 LW-3/D3 落地触发审(mergeGameRules formula_overrides 白名单 damage|defense|crit)待 🧭 转。待 🧭 批准后复验 ui_unlocks 守卫执行。
