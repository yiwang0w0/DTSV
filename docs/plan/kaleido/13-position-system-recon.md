...
2. **恢复周期是「参数」不是「档位」** —— ⚠ **本条已按 🧭 转达的 Kanata 原话订正（2026-07-23）**，我原先提的两档枚举 `reset_scope:'daily'|'permanent'` **被否**：
   ...
   已定三例：**安全屋的门 = `'infinity'`（永不·存档点锚·当前唯一持有者）** / 灯 = 次日 / 炸毁区块 = 数天后。

   ⚠ **本条的理由换过一次，记下来免得后人以为是随口定的**：
   我最初反对「`NULL` = 永不」的理由是「NULL 在『未设置』与『永不』之间有歧义」—— **这条已由我自己撤回**：本表的行**只有被写入时才存在**，「未设置」不落行，故 NULL 无歧义。
   **真正的否决理由是 🧭 实测出的读侧不对称**：

   | 表达式 | DELETE 侧 `restore_at <= now()` | 读侧 `restore_at > now()`（「仍有效」） |
   |---|---|---|
   | `'infinity'` | false → 扫不到 ✓ | true → 读得到 ✓ |
   | `NULL` | 不匹配 → 扫不到 ✓ | **不匹配 → 永久行被静默丢掉** ✗ |

   ⇒ NULL **只在删除侧安全**；而读侧过滤恰恰是每次读场景状态都要写的那一步，`WHERE restore_at > now()` 是最自然的写法 —— **门就永远读不出来**。
   即「把存档点锚弄丢」这个后果原样发生，只是从 `DELETE` 挪到了 `SELECT`。`'infinity'` 两侧行为都正常，且三值逻辑一处都不用记。

   DDL = `scripts/kaleido-scene-state.sql`（**已写·未跑·待 🧭 恢复后审**）。
