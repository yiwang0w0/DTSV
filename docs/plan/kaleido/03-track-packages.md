...
# KP1-R 恢复令重排(2026-07-07 深夜 · Kanata 解禁 · 05 渐进披露冲击落单)

> 依据 `05-progressive-disclosure.md` §2 冲击评估。本段只**重排/增补**上文 KP1-E/G/C/X/N,不推翻;冲突处以本段为准。跨轨接口一律经 🧭 仲裁后广播。

## 🔧 引擎 KP1-E(重排:ui_unlocks 提到首位)
0. **【新增·最优先】ui_unlocks 机制**(05 §1 接口形状):①触发判定挂路由边界,复用传感层动词(search/fight_start/level_clear/item_use/craft_attempt/death…,timing before|after) ②**账号级持久化落库形状定稿**(profiles jsonb 列 vs 轻表 player_ui_state 二选一,DDL 幂等 SQL 先交 🧭/🔒 审再经 postgres MCP 执行) ③解锁事件随 action 响应下发(含 nar_line,文案走数据) ④E2E 增「解锁序 + 硬时序法则(hp_bar 先于首害)」断言。**接口形状(客户端可见的解锁集字段 + 解锁事件 payload)定稿后立刻报 🧭 广播给 🎨**——🎨 结构级改造等这个形状,别让它裸奔。
1-3. LW-3 / D3 / D5 照旧队列,排在 ui_unlocks 接口形状定稿之后(实现可穿插)。

## 🎨 前端 KP1-C v2(结构级改造)
1. **kaleido 全部 UI 件改为解锁物**:条件渲染包裹(关卡头/R6 卡/背包/战斗面板/移动按钮/回合计数…按 05 §1.3 清单);初始态 = **一个搜索按钮**;浮现配渐次动效 + 该 ui_key 的 nar_line 落日志。
2. **先按 05 §1.1 形状 + 本地 stub 开工**(条件渲染骨架/动效/初始态),🔧 接口形状广播后接真数据——不空等。
3. 1b 三态 UI 归入 stance_ui 解锁物;12 项 ui_key 清单的前端校订意见(可行性/遗漏)回 🧭。
4. 红线:多人局渲染路径零改动;文案零硬编码(nar_line 来自数据)。

## ⚙️ 内容数值 KP1-G(照旧首单 + 解锁链约束)
- D6 种子关结构照旧第一优先,**新增硬约束**:解锁触发链须在 seq1-2 自然发生(首道具/首遭遇/首配方材料的投放下限,05 §2)——12-15 关排布时自查。
- **D6 结构初稿产出后第一时间给 📖**(N4 文案依赖你的结构,别攒到全量才交)。
- 平衡核算/敌人道具池/掉落表/合成树照旧队列。

## 📖 剧情(轻载待接)
1. 自查 N3 供稿与 05 §1.3 十二项 ui_key 清单逐项对齐(缺口补 nar_line,含 timing=before 条目的时序措辞)。
2. 收到 ⚙️ D6 结构初稿即启动 N4 种子关文案(名字/描述/入关文本/敌人命名/氛围行)。
3. 四项 Kanata 挂起决策不阻塞以上两项;期间结局文案维持冻结。

## 🔒 安全 KP1-X(即可开工项 + 新审点)
1. **现在可做**:kaleido-e2e.mjs 安全复核(凭证读取面/清理完备性,低优)。
