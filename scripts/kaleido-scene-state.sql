...
CREATE TABLE IF NOT EXISTS kaleido_scene_state (
  ...
  --   为什么不是 `NULL = 永不`（🔧 原提案 —— 该提案的**原始理由已被 🔧 自己撤回**：本表的行只有被写入时
  --     才存在，「未设置」不落行，故 NULL 无歧义）。**真正的否决理由是 🧭 实测出的读侧不对称**：
  --       表达式                 DELETE 侧 `restore_at <= now()`      读侧 `restore_at > now()`（"仍有效"）
  --       'infinity'             false → 扫不到 ✓                     true  → 读得到 ✓
  --       NULL                   不匹配 → 扫不到 ✓                    **不匹配 → 永久行被静默丢掉** ✗
  --     ⇒ NULL 只在删除侧安全；而「读侧过滤」恰恰是每次读场景状态都要写的那一步，
  --       `WHERE restore_at > now()` 是最自然的写法 —— **门就永远读不出来**。
  --       即「把存档点锚弄丢」这个后果原样发生，只是从 DELETE 挪到了 SELECT。
  --     ⇒ `'infinity'` 两侧行为都正常，且三值逻辑一处都不用记。
